...
import { MusicContext } from '../App'; // 1. Import Context Musik
...
function Question() {
  ...
  // 2. Ambil fungsi playMusic dari Context
  const { playMusic } = useContext(MusicContext);

  // 3. Jalankan musik otomatis saat komponen dimuat
  useEffect(() => {
    playMusic();
  }, [playMusic]);
      ...
      {!isInputFocused ? (
        <div className="flex flex-col items-center px-4 pt-16">
          ...
          >
            ...
            <input
              ...
              readOnly // Mencegah keyboard muncul sebelum fokus benar-benar pindah
            />
            ...
            <div className="space-y-4">
              {config.trendingSearches.map((trend) => (
                <div 
                  ...
                  onClick={() => setSearchQuery(trend)} // Opsional: Klik tren untuk isi search
                >
                  <Search className="w-5 h-5 text-[#9aa0a6]" />
                  <span>{trend}</span>
                </div>
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto ">
          ...
          <div className="mt-10 px-5 text-white/50">
            <p className="text-sm italic">{config.proTip}</p>
          </div>
        </div>
      )}
    </div>
